...
<!-- Modal Remove Admin  -->
			...
			<div class="main-panel">
				<input class="form-control mt-3 mb-2 ml-4" id="searchInput" type="text" placeholder="Search" aria-label="Search" style="width: 96%">
					...
					<div class="d-flex justify-content-between align-items-center">
						<h4 class="page-title mt-2">Daftar Paket Wisata</h4>
						<a href="{{ route('admin.tambahPaket') }}" class="btn btn-primary">Tambah Paket Wisata</a>	
					</div>				
						<div class="row">
							<div class="col">
								<div class="card">
									<div class="card-body">
										<table class="table table-striped mt-3" id="paketTable">
											<thead>
												<tr>
													<th scope="col">Nama Paket</th>
													<th scope="col">Tempat Wisata</th>
													<th scope="col">Tentang Paket</th>
													<th scope="col">Gambar Paket</th>
    										...
    										@foreach ($paket_wisata as $itemPaket)
        										<tr>
            										<td>{{ $itemPaket->nama_paket }}</td>
													<td>{{ $itemPaket->nama_tempat }}</td>
            										<td>{!! Str::limit($itemPaket->tentang_paket, 40, '...') !!} <a href="#" data-bs-toggle="modal" data-bs-target="#modalView_Paket{{$itemPaket->id_paket}}">View more..</a></td>
													</td>
            										<td>
                									@if ($itemPaket->gambar_paket)
                									    <img src="{{ asset('storage/' . $itemPaket->gambar_paket) }}" width="100" height="80" alt="">
                									@else
                									    Tidak ada gambar
                									@endif
													</td>
													<td>By {{ $itemPaket->name }} at {{ $itemPaket->updated_at }}</td>
													<td><a href="{{ route('admin.editPaket', $itemPaket->id_paket) }}">Edit</a> | <a href="" data-bs-toggle="modal" data-bs-target="#modal_confirmation_{{ $itemPaket->id_paket }}">Hapus</a></td>
												</tr> 

												<!-- Modal View Berita -->
		  										<div class="modal fade" id="modalView_Paket{{$itemPaket->id_paket}}" tabindex="-1" aria-labelledby="modalTitle{{$itemPaket->id_paket}}" aria-hidden="true">
   													<div class="modal-dialog">
        												<div class="modal-content">
            												<div class="modal-header">
																<h5 class="modal-title" id="modalTitle">Detail Paket Wisata</h5>
																<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            												</div>
            												<div class="modal-body">
															<div>
                                							    <p class="mb-3">Nama Paket Wisata: {{ $itemPaket->nama_paket }}</p>
																<p class="mb-3">Letak Wisata: {{ $itemPaket->nama_tempat }}</p>
																<img src="{{ asset('storage' . $itemPaket->gambar_paket) }}" alt="">
                                							    <p class="my-3">{!! $itemPaket->tentang_paket !!}</p>
                                							</div>
															</div>
												        </div>
												    </div>
												...
												<div class="modal fade" id="modal_confirmation_{{ $itemPaket->id_paket }}" tabindex="-1" aria-labelledby="modalTitle" aria-hidden="true">
   													<div class="modal-dialog">
    													<div class="modal-content">
    														<div class="modal-header">
    														...
    														<div class="modal-body">
																...
																<div class="d-flex justify-content-end">
																	...
																	<a href="{{ route('admin.deletePaket', $itemPaket->id_paket) }}" class="btn btn-danger ml-3">Hapus</a>
																</div>
												            </div>
												        </div>
												    </div>
	...
	document.getElementById("searchInput").addEventListener("keyup", function() {
	    ...
	    const rows = document.querySelectorAll("#paketTable tbody tr");
